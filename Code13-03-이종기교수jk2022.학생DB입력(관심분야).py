...
def insertData() :
    ...
    con = sqlite3.connect("C:/sqlite/jk2022")  # DB가 저장된 폴더까지 지정
    cur = con.cursor()

    data1 = edt1.get(); data2 = edt2.get(); data3 = edt3.get(); data4 = edt4.get(); data5 = edt5.get(); data6 = edt6.get()
    try :
        sql = "INSERT INTO userTable2 VALUES('" + data1 + "','" + data2 + "','" + data3 + "','" + data4 + "', '" + data5 + "', '" + data6 + "')"
        cur.execute(sql)
    except :
...
def selectData() :
    ...
    strData3.append("이메일"); strData4.append("출생연도"); strData5.append("전화번호"); strData6.append("관심분야");
...
window.title("GUI 이종기교수 'JK2022' 학생DB입력(관심분야)")
